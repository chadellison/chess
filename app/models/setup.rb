class Setup < ApplicationRecord
  ...
  SIGNATURES = [
    ...
    'white_threat',
    'black_threat',
    'white_pawn_attackttack',
    'black_pawn_attackttack',
    'white_knight_attack',
    'black_knight_attack',
    'white_bishop_attack',
    'black_bishop_attack',
  ...
  def add_signatures(new_pieces, game_turn_code)
    SIGNATURES.each do |signature_type|
      if signatures.find_by(signature_type: signature_type).blank?
        ...
        Signature.find_or_create_by(value: signature, signature_type: signature_type).setups << self if signature.present?
      end
    end
  end
end
