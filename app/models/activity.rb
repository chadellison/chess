class Activity
  def self.create_abstraction(position_data)
    ...
    if in_check?(new_pieces)
      ...
      move_count = count_moves(pieces, new_pieces, position_data)
      next_move_count = count_moves(new_pieces, pieces, position_data)
      (next_move_count - move_count) * 0.1
    end
  end

  def self.count_moves(first_set, second_set, position_data)
      ...
      move_count = piece.valid_moves.count do |move|
        second_set.none? do |next_piece|
          if next_piece.piece_type.downcase == 'p'
            position_data.pawn_attack(next_piece).include?(move)
          else
