class NeuralNetwork
  attr_reader :inputs, :desired_outputs
  ...
  def calculate_prediction(abstraction)
    input = convert_to_input(abstraction)
    saved_nn = RubyFann::Standard.new(filename: "#{Rails.root}/verify.train")
    saved_nn.run(input)
  end

  def load_training_data(count)
    Abstraction.order('RANDOM()').limit(count).find_each do |abstraction|
      @inputs << convert_to_input(abstraction)
      @desired_outputs << calculate_outcomes(abstraction)
    end
  end

  def train(num_inputs, hidden_neurons, num_outputs)
  ...
  # end
  #
  def convert_to_input(abstraction)
