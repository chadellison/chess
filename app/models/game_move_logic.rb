class GameMoveLogic
  ...
  class << self
    def find_next_moves(pieces, turn, move_count)
      ...
      json_moves = REDIS.get(key)

      if json_moves.present?
        JSON.parse(json_moves).map { |move| MoveSerializer.deserialize(move) }
      else
        next_moves = pieces.select { |piece| piece.color == turn }.map do |piece|
          all_next_moves_for_piece(piece, turn, move_count, pieces)
        end.flatten
        REDIS.set(key, next_moves.map { |move| MoveSerializer.serialize(move) })
